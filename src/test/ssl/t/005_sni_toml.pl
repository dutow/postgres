...
my $toml_rules = $node->safe_psql('postgres',
	"SELECT count(*) FROM pg_hosts_file_rules() WHERE error IS NULL");
ok($toml_rules >= 1, "pg_hosts_file_rules: valid TOML entries reported");
...
my $toml_errs = $node->safe_psql('postgres',
	"SELECT count(*) FROM pg_hosts_file_rules() WHERE error IS NOT NULL");
ok($toml_errs >= 1, "pg_hosts_file_rules: malformed TOML becomes an error row");
