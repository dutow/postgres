...
CREATE TABLE errtst(a int PRIMARY KEY, b int CHECK (b < 10),
  secret text DEFAULT 'top secret');
GRANT SELECT (a) ON TABLE errtst TO regress_priv_user2;
GRANT INSERT (a, b, secret) ON TABLE errtst TO regress_priv_user2;
...
-- nor may one INSERT action's column list exempt columns of a row another
-- INSERT action produced
MERGE INTO errtst USING (VALUES (2)) v(a) ON errtst.a = v.a
  WHEN NOT MATCHED AND v.a = 0 THEN INSERT (a, b, secret) VALUES (v.a, 1, 'mine')
  WHEN NOT MATCHED THEN INSERT (a, b) VALUES (v.a, 10);
