		...
		if (violationOK)
		{
			...
			if (conflictTid)
			{
				*conflictTid = existing_slot->tts_tid;
				 ...
				 * record it: that takes the SIREAD lock and checks for a
				 * concurrent writer of the tuple in one step, which a
				 * predicate lock acquired separately after the scan could not
				 * do.  The result is of no interest here, the caller checks
				 * visibility itself.
				 */
