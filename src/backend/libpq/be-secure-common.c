 ...
 * values into *out (a HostsLine). String pointers reference the parsed TOML
 * document and must be pstrdup'd by the caller if they must outlive it.
 * "label" is the dotted path for diagnostics, e.g. "hosts._defaults".
 *
...
hosts_toml_collect(toml_datum_t tab, const char *label, const char *filename,
				   HostsLine *out, bool *reload_present, char **err_msg)
{
	...
	*reload_present = false;
	...
	if (!toml_reject_unknown(hosts_toml_keys, tab, label, filename, err_msg))
		return false;

	if (cert.type != TOML_UNKNOWN && cert.type != TOML_STRING)
		return toml_type_error(label, "ssl_certificate", "string", filename, err_msg);
	if (key.type != TOML_UNKNOWN && key.type != TOML_STRING)
		return toml_type_error(label, "ssl_key", "string", filename, err_msg);
	if (ca.type != TOML_UNKNOWN && ca.type != TOML_STRING)
		return toml_type_error(label, "ssl_ca", "string", filename, err_msg);
	if (cmd.type != TOML_UNKNOWN && cmd.type != TOML_STRING)
		return toml_type_error(label, "passphrase_command", "string", filename, err_msg);
	if (rel.type != TOML_UNKNOWN && rel.type != TOML_BOOLEAN)
		return toml_type_error(label, "passphrase_command_reload", "boolean", filename, err_msg);
	...
	*file_err = NULL;
	...
	if (defaults_dat.type == TOML_TABLE)
	{
		...
		if (hosts_toml_collect(defaults_dat, "hosts._defaults", filename,
							   &defaults, &defaults_reload_present, &derr))
			have_defaults = true;
		else
		{
			HostsLine  *hl = palloc0_object(HostsLine);

			hl->err_msg = derr;
			hl->linenumber = defaults_dat.lineno;
	...
	else if (defaults_dat.type != TOML_UNKNOWN)
	{
		...
		hl->err_msg = psprintf("\"hosts._defaults\" in \"%s\" must be a table",
							   filename);
		hl->linenumber = defaults_dat.lineno;
		ereport(elevel,
				(errcode(ERRCODE_CONFIG_FILE_ERROR),
	...
	for (int i = 0; i < hosts_section.u.tab.size; i++)
	{
		...
		{
			hl->err_msg = psprintf("\"hosts.%s\" in \"%s\" must be a table",
								   name, filename);
			ereport(elevel,
					(errcode(ERRCODE_CONFIG_FILE_ERROR),
...

		{
			...
			bool		ok = hosts_toml_collect(val, label, filename, hl,
												&reload_present, &eerr);
		...
		{
			hl->err_msg = psprintf("\"hosts.%s\" in \"%s\" is missing required \"ssl_certificate\" or \"ssl_key\"",
								   name, filename);
			ereport(elevel,
					(errcode(ERRCODE_CONFIG_FILE_ERROR),
...
load_hosts(List **hosts, char **err_msg)
{
	...
	*hosts = NIL;
	...
	if (toml_path(HostsFileName))
	{
		...
		entries = parse_hosts_toml(HostsFileName, LOG, &missing, &file_err);
		if (missing)
