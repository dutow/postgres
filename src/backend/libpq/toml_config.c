 ...
 * the dotted path for diagnostics (e.g. "hosts.foo"); each offender is
 * reported at elevel. Returns true if all keys known.
 */
bool
toml_reject_unknown(const char *const allowed[], toml_datum_t tab,
					const char *label, const char *filename, int elevel)
{
	bool		ok = true;
...
/*
 * Report that "label.field" in "filename" must be of type "expected" and
 * return false for use as a one-line bail-out.
 */
bool
toml_type_error(const char *label, const char *field, const char *expected,
				const char *filename, int elevel)
{
	ereport(elevel,
			(errcode(ERRCODE_CONFIG_FILE_ERROR),
